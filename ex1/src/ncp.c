...
typedef struct { //this should be defined in a header file so both can use it.
...
void packet_maker(packet *x, int packet_type, int index, int size, char *data_buffer){

char bit1 = 0x80;
char bit2 = 0x40;
char bit3 = 0x20;
 
char *header_byte_1 = x;
char *header_byte_2 = x+1;
char *header_byte_3 = x+2;
char *header_byte_4 = x+3;

char *data = x +4;
	if (packet_type = 1){ //packet_type = 1 is hello
*header_byte_1 = 0x00;
*header_byte_1 = *header_byte_1 | bit1;
	} else (packet_type = 2){ //packet_type = 2 is data
*header_byte_1 = 0x00;
